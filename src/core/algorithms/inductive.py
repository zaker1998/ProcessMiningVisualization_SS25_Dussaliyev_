...
from core.log_processing.splits import (
    exclusive_split,
    parallel_split,
    sequence_split,
    loop_split,
)
from core.graphs.cuts import exclusive_cut, parallel_cut, sequence_cut, loop_cut
from core.graphs.dfg import DFG
from core.graphs.visualization.inductive_graph import InductiveGraph
from core.log_processing.filters import filter_events, filter_traces
...
class InductiveMining(BaseMining):
    ...
    def calulate_cut(self, log) -> tuple | None:
        ...
        if partitions := exclusive_cut(dfg):
            return ("xor", *exclusive_split(log, cast(List[Set[str]], partitions)))
        elif partitions := sequence_cut(dfg):
            return ("seq", *sequence_split(log, cast(List[Set[str]], partitions)))
        elif partitions := parallel_cut(dfg):
            return ("par", *parallel_split(log, cast(List[Set[str]], partitions)))
        elif partitions := loop_cut(dfg):
            return ("loop", *loop_split(log, cast(List[Set[str]], partitions)))
