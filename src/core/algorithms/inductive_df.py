"""
Inductive Miner - Directly-Follows (IMd) - Paper-Based Implementation
...
This implementation follows the paper-based algorithm specification from the 2018 paper.
"""
...
class InductiveMiningDF(InductiveMining):
    """
    Paper-based implementation of Inductive Miner - Directly-Follows (IMd).
    ...
    def __init__(self, log: Dict[Tuple[str, ...], int]):
        ...
        logger.info("Initialized IMd (Inductive Miner - Directly-Follows) with paper-based algorithm")
    ...
    def generate_graph(
        ...
        This method applies pre-filtering and then runs the paper-based IMd algorithm.
    ...
    def calculate_cut(self, log: Dict[Tuple[str, ...], int]) -> Optional[Tuple[str, List[Dict[Tuple[str, ...], int]]]]:
        """
        Paper-based IMd cut detection using DFG-based approach.
        ...
        Paper-based IMd with Optional Edge Filtering:
        ...
        try:
            ...
            # Apply optional edge filtering if configured (PAPER-BASED)
            ...
            # PAPER-BASED: Use filtered DFG for detection, but split original log
    ...
    def _try_all_cuts_dfg(
        ...
        Cut Detection Order (paper-based):
        ---------------------------------
        ...
        log : Dict[Tuple[str, ...], int]
            ...
            Note: In paper-based IMd, this would be DFG projection, not log splitting
        ...
        for operator, cut_func, split_func, description in cut_attempts:
            try:
                ...
                if partitions and len(partitions) > 1:
                    ...
                    # (In paper-based IMd, this would be DFG projection)
    ...
    def _create_filtered_dfg(self, log: Dict[Tuple[str, ...], int]) -> DFG:
        """
        Create filtered DFG with optional edge cutoff (Paper approach).
        
        This is an OPTIONAL extension to handle noisy logs when needed.
        
        Algorithm (from IMf paper, adapted for IMd):
        ------------------------------------------------------
        ...
        # Calculate threshold (PAPER-BASED)
        ...
        # Create filtered DFG (PAPER-BASED)
        filtered_dfg = DFG()
        
        # Add ALL nodes (preserve all activities) - PAPER-BASED requirement
        ...
        # Preserve start and end node information (PAPER-BASED)
    ...
    def _preserve_start_end_nodes(self, dfg: DFG, log: Dict[Tuple[str, ...], int]):
        ...
        IMPORTANT: In the paper-based algorithm, start/end node information is
