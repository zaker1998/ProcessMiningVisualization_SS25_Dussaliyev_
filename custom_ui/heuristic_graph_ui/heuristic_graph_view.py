...
class HeuristicGraphView(QWidget, AlgorithmViewInterface):
    def __init__(self, parent, saveFolder="saves/", workingDirectory="temp/graph_viz"):
        ...
        self.HeuristicGraphController = HeuristicGraphController(
            workingDirectory, self.dependency_threshold, self.min_frequency
        )
        self.graph = None  # the graph object
        ...
        self.saveProject_button = SaveProjectButton(
            self.parent, self.saveFolder, self.getModel
        )
        self.export_button = ExportButton(self.parent)
        slider_frame_layout = QVBoxLayout()
        slider_frame_layout.addWidget(
            QLabel("Heuristic Mining Modifiers", alignment=Qt.AlignCenter)
        )
    ...
    def loadModel(self):
        try:
            file_path, _ = QFileDialog.getOpenFileName(
                None, "Select file", self.saveFolder, "Pickle files (*.pickle)"
            )
    ...
    def __mine_and_draw(self):
        self.graph = self.HeuristicGraphController.create_dependency_graph(
            self.dependency_threshold, self.min_frequency
        )
    ...
    def __ensure_graph_exists(self):
        # just to make sure everything works as intended.
        if not self.graph:
            print("HeuristicGraphView ERROR: graph is NONE.")
            return False
        return True

    def generate_png(self):
        if not self.__ensure_graph_exists():
            return
        self.graph.export_graph(self.workingDirectory, format="png")
    ...
    def generate_svg(self):
        if not self.__ensure_graph_exists():
            return
        self.graph.export_graph(self.workingDirectory, format="svg")
        print("heuristic_graph_view: SVG generated")

    def generate_dot(self):
        if not self.__ensure_graph_exists():
            return
        self.graph.export_graph(self.workingDirectory, format="dot")
