...
class FuzzyGraphView(QWidget, AlgorithmViewInterface):
    def __init__(self, parent, saveFolder="saves/", workingDirectory="temp/graph_viz"):
        ...
        self.FuzzyGraphController = FuzzyGraphController(
            workingDirectory, self.significance, self.edge_cutoff, self.utility_ratio
        )
        ...
        self.edge_cutoff_slider = CustomQSlider(
            self.__edge_cutoff_slider_changed, Qt.Vertical
        )
        ...
        slider_frame_layout.addWidget(
            QLabel("<b>Fuzzy Mining Modifiers</b>", alignment=Qt.AlignCenter)
        )
    ...
    def __redraw(self):
        self.graphviz_graph = self.FuzzyGraphController.mine_and_draw(
            self.significance, self.correlation, self.edge_cutoff, self.utility_ratio
        )
    ...
    def generate_dot(self):
        if not self.__check_if_graph_exists():
            return
        self.graphviz_graph.export_graph(self.workingDirectory, format="dot")
    ...
    def generate_svg(self):
        if not self.__check_if_graph_exists():
            return
        self.graphviz_graph.export_graph(self.workingDirectory, format="svg")
    ...
    def generate_png(self):
        if not self.__check_if_graph_exists():
            return
        self.graphviz_graph.export_graph(self.workingDirectory, format="png")
