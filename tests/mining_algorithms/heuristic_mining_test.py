...
class TestHeuristic(unittest.TestCase):
    ...
    def __check_graph_integrity_with_netx(self, dotsource):
        ...
        assert (
            nx_graph.out_degree(start_node) >= 1
        ), "The 'start' node does not connect to any other nodes."

        # Check there is an 'end' node
        end_node = None
        for node in nx_graph.nodes:
            if (
                nx_graph.out_degree(node) == 0
            ):  # Out-degree of 0 indicates an 'end' node
                end_node = node
                break
        ...
        assert set(reachable_nodes) == set(
            all_nodes
